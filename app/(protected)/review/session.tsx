...
export default function ReviewSessionScreen() {
  ...
  const progress = useMemo(() => words.length ? `${Math.min(index + 1, words.length)} / ${words.length}` : '0 / 0', [index, words.length]);
  ...
  return (
    <Screen className="bg-slate-950 pt-6">
      ...
      <View className="mt-4 h-2 overflow-hidden rounded-full bg-slate-800">
        <View className="h-full bg-primary-500" style={{ width: `${((index + 1) / words.length) * 100}%` }} />
      </View>
      <View className="flex-1 justify-center">
        <Card className="min-h-[360px] justify-center gap-4 bg-slate-900" onTouchEnd={() => setRevealed(true)}>
